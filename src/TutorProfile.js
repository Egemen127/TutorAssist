...
function TutorProfile(){
    ...
    <List>{tutor.courses?.map(e=>(<><ListItem><ListItemText primary={e.courseName} secondary={`There are ${e.students.length} students in this class`}/><ListItemText  secondary={`Starts at: ${e.startDate} Ends at: ${e.endDate}`}/><Button>Register</Button></ListItem><Divider /></>))}</List>
    <Button onClick={handleOpen}>Send a message to tutor</Button>
    <Modal open={open}  onClose={handleClose} ><Card style={style} component="form">
        <TextField
        ...
        /><Divider/><Button style={{margin:'auto'}}>Send</Button></Card></Modal>
    </div>
